const {
  france
} = require('../framework/france');
var mumaker = require("mumaker");
france({
  'nomCom': "hacker",
  'categorie': "Logo",
  'reaction': "👨🏿‍💻"
}, async (_0xd36572, _0x4cc7d9, _0x4da9f5) => {
  const {
    prefixe: _0x34f139,
    arg: _0x17e73d,
    ms: _0x4243fc,
    repondre: _0x7fa6fd
  } = _0x4da9f5;
  if (!_0x17e73d || _0x17e73d == '') {
    _0x7fa6fd("*__Exemple : * " + _0x34f139 + "hacker France");
    return;
  }
  try {
    let _0x306ea0 = await mumaker.ephoto('https://en.ephoto360.com/create-anonymous-hacker-avatars-cyan-neon-677.html', _0x17e73d);
    _0x7fa6fd('*processing...*');
    await _0x4cc7d9.sendMessage(_0xd36572, {
      'image': {
        'url': _0x306ea0.image
      },
      'caption': "\t *Logo by FLASH-MD*"
    }, {
      'quoted': _0x4243fc
    });
  } catch (_0x1cc018) {
    _0x7fa6fd("🥵🥵 " + _0x1cc018);
  }
});
france({
  'nomCom': "sand",
  'categorie': 'Logo',
  'reaction': '⌛'
}, async (_0x2a12ea, _0x3cd2d4, _0x2bce4a) => {
  const {
    prefixe: _0x4a45eb,
    arg: _0x237137,
    ms: _0x2c8d47,
    repondre: _0x796190
  } = _0x2bce4a;
  if (!_0x237137 || _0x237137 == '') {
    _0x796190("*Example Usage : * " + _0x4a45eb + "sand Flash-Md");
    return;
  }
  try {
    let _0x3eb776 = await mumaker.ephoto("https://en.ephoto360.com/write-names-and-messages-on-the-sand-online-582.html", _0x237137);
    _0x796190("*Flash-Md is Processing...*");
    await _0x3cd2d4.sendMessage(_0x2a12ea, {
      'image': {
        'url': _0x3eb776.image
      },
      'caption': "\t *Logo By FLASH-MD*"
    }, {
      'quoted': _0x2c8d47
    });
  } catch (_0x3ef129) {
    _0x796190("🚫🚫 " + _0x3ef129);
  }
});
france({
  'nomCom': "dragonball",
  'categorie': "Logo",
  'reaction': '🐉'
}, async (_0x141807, _0x5408ed, _0x52f99f) => {
  let {
    arg: _0x3bca62,
    repondre: _0x28a782,
    prefixe: _0x2ff7d5,
    ms: _0x553819
  } = _0x52f99f;
  try {
    const _0x4d9fd1 = "*_EXEMPLE *:  " + _0x2ff7d5 + "dragonball FranceKing";
    if (_0x3bca62 == '' || !_0x3bca62) {
      _0x28a782(_0x4d9fd1);
      return;
    }
    const _0x1073c6 = await mumaker.ephoto("https://en.ephoto360.com/create-dragon-ball-style-text-effects-online-809.html", _0x3bca62.join(" "));
    await _0x5408ed.sendMessage(_0x141807, {
      'text': " *\t Traitement en cours ...*"
    }, {
      'quoted': _0x553819
    });
    await _0x5408ed.sendMessage(_0x141807, {
      'image': {
        'url': _0x1073c6.image
      },
      'caption': " \t *Logo by FLASH-MD*"
    }, {
      'quoted': _0x553819
    });
  } catch (_0x194dd3) {
    _0x28a782("🥵🥵 " + _0x194dd3);
  }
});
france({
  'nomCom': "naruto",
  'categorie': "Logo",
  'reaction': '⛩'
}, async (_0x1cd9ad, _0x1d5bd, _0x64fc0b) => {
  let {
    ms: _0x322727,
    arg: _0x421d25,
    repondre: _0x48a3d9,
    prefixe: _0x4206c3
  } = _0x64fc0b;
  try {
    if (!_0x421d25 || _0x421d25 == '') {
      _0x48a3d9("*_Exemple : * " + _0x4206c3 + "naruto FLASH-MD");
      return;
    }
    _0x48a3d9("*traitement en cours...*");
    var _0x45c6e5 = await mumaker.ephoto('https://en.ephoto360.com/naruto-shippuden-logo-style-text-effect-online-808.html', _0x421d25.join(" "));
    await _0x1d5bd.sendMessage(_0x1cd9ad, {
      'image': {
        'url': _0x45c6e5.image
      },
      'caption': "\t *Logo by FLASH-MD*"
    }, {
      'quoted': _0x322727
    });
  } catch (_0x2d1bcb) {
    _0x48a3d9("🥵🥵 " + _0x2d1bcb);
  }
});
france({
  'nomCom': "didong",
  'categorie': 'Logo',
  'reaction': '📱'
}, async (_0x24d0c9, _0x2481cf, _0x234440) => {
  try {
    if (!_0x181394 || _0x181394 == '') {
      _0x248cc8("*exemple :* " + _0x16cabd + "didong France King");
      return;
    }
    var _0x4d57d1 = await mumaker.ephoto("https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html", _0x181394.join(" "));
    _0x248cc8("*processing...*");
    await _0x2481cf.sendMessage(_0x24d0c9, {
      'image': {
        'url': _0x4d57d1.image
      },
      'caption': "\t *Logo by FLASH-MD*"
    });
  } catch (_0x18f72d) {
    _0x248cc8("🥵🥵 " + _0x18f72d);
  }
});
france({
  'nomCom': "wall",
  'categorie': 'Logo',
  'reaction': '👍'
}, async (_0x4a29e7, _0xd75be3, _0x11aa3e) => {
  const {
    arg: _0x5ecc77,
    repondre: _0x51b26b,
    ms: _0x2f1fb6,
    prefixe: _0x25ea07
  } = _0x11aa3e;
  if (!_0x5ecc77[0x0]) {
    _0x51b26b("Exemple of using commande:\n " + _0x25ea07 + "wall FLASH-MD");
    return;
  }
  let _0x516dcd = _0x5ecc77.join(" ");
  mumaker.textpro("https://textpro.me/break-wall-text-effect-871.html", _0x516dcd).then(_0x18f4e2 => {
    _0xd75be3.sendMessage(_0x4a29e7, {
      'image': {
        'url': _0x18f4e2.image
      },
      'caption': "Logo by *France King*"
    }, {
      'quoted': _0x2f1fb6
    });
  })["catch"](console.log);
});
france({
  'nomCom': "summer",
  'categorie': "Logo",
  'reaction': '🌞'
}, async (_0x7d657e, _0x1bd92d, _0x11435e) => {
  const {
    arg: _0x3be5f2,
    repondre: _0x37f281,
    ms: _0x565ce8,
    prefixe: _0x24ee2b
  } = _0x11435e;
  if (!_0x3be5f2[0x0]) {
    _0x37f281("Exemple of using commande:\n " + _0x24ee2b + "summer My text");
    return;
  }
  const _0x3fa918 = _0x3be5f2.join(" ");
  mumaker.textpro('https://textpro.me/create-sunset-light-text-effects-online-for-free-1124.html', _0x3fa918).then(_0x39bf1e => {
    _0x1bd92d.sendMessage(_0x7d657e, {
      'image': {
        'url': _0x39bf1e.image
      },
      'caption': "Logo by *FLASH-MD*"
    }, {
      'quoted': _0x565ce8
    });
  })["catch"](console.error);
});
france({
  'nomCom': "neonlight",
  'categorie': "Logo",
  'reaction': '💡'
}, async (_0x144c66, _0x3fdda4, _0x19c522) => {
  const {
    arg: _0x358d9c,
    repondre: _0x151171,
    ms: _0x1127a5,
    prefixe: _0x40b08a
  } = _0x19c522;
  if (!_0x358d9c || _0x358d9c == '') {
    _0x151171("Exemple of using commande:\n " + _0x40b08a + "neonlight My text");
    return;
  }
  try {
    let _0x45cb2e = await mumaker.textpro("https://textpro.me/create-glowing-neon-light-text-effect-online-free-1061.html", _0x358d9c);
    _0x3fdda4.sendMessage(_0x144c66, {
      'image': {
        'url': _0x45cb2e.image
      },
      'caption': "Logo by *FLASH-MD*"
    }, {
      'quoted': _0x1127a5
    });
  } catch (_0x28c5ba) {
    _0x151171("🥵🥵 " + _0x28c5ba);
  }
});
france({
  'nomCom': "greenneon",
  'categorie': 'Logo',
  'reaction': '🟢'
}, async (_0x2d32ca, _0x387de4, _0x56a8d2) => {
  const {
    arg: _0x5ac43d,
    repondre: _0x6979a0,
    ms: _0xf9096b,
    prefixe: _0x2ba19c
  } = _0x56a8d2;
  if (!_0x5ac43d[0x0]) {
    _0x6979a0("Exemple of using commande:\n " + _0x2ba19c + "greenneon My text");
    return;
  }
  const _0x5ab78f = _0x5ac43d.join(" ");
  mumaker.textpro("https://textpro.me/green-neon-text-effect-874.html", _0x5ab78f).then(_0x371f0b => {
    _0x387de4.sendMessage(_0x2d32ca, {
      'image': {
        'url': _0x371f0b.image
      },
      'caption': "Logo by *France King*"
    }, {
      'quoted': _0xf9096b
    });
  })["catch"](console.error);
});
france({
  'nomCom': 'glitch',
  'categorie': "Logo",
  'reaction': "🎛️"
}, async (_0x263baf, _0x43a1ff, _0x23e3bd) => {
  const {
    arg: _0x262085,
    repondre: _0x5c0330,
    ms: _0x1cb9ba,
    prefixe: _0x528a55
  } = _0x23e3bd;
  if (!_0x262085[0x0]) {
    _0x5c0330("Exemple of using commande:\n " + _0x528a55 + "glitch My text");
    return;
  }
  const _0x127a98 = _0x262085.join(" ");
  mumaker.textpro("https://textpro.me/create-impressive-glitch-text-effects-online-1027.html", _0x127a98).then(_0x42c768 => {
    _0x43a1ff.sendMessage(_0x263baf, {
      'image': {
        'url': _0x42c768.image
      },
      'caption': "Logo by *France King*"
    }, {
      'quoted': _0x1cb9ba
    });
  })["catch"](console.error);
});
france({
  'nomCom': "devil",
  'categorie': "Logo",
  'reaction': '😈'
}, async (_0x468813, _0x3c0b3e, _0x36932c) => {
  const {
    arg: _0x2bf3a5,
    repondre: _0x52145c,
    ms: _0x556f94,
    prefixe: _0x218f0d
  } = _0x36932c;
  if (!_0x2bf3a5[0x0]) {
    _0x52145c("Exemple of using commande:\n " + _0x218f0d + "devil My text");
    return;
  }
  mumaker.textpro('https://textpro.me/create-neon-devil-wings-text-effect-online-free-1014.html', _0x2bf3a5).then(_0xe3ecc4 => {
    _0x3c0b3e.sendMessage(_0x468813, {
      'image': {
        'url': _0xe3ecc4.image
      },
      'caption': "Logo by *FLASH-MD*"
    }, {
      'quoted': _0x556f94
    });
  })['catch'](console.error);
});
france({
  'nomCom': 'boom',
  'categorie': 'Logo',
  'reaction': '💥'
}, async (_0x117e9b, _0x154a8e, _0x3594e9) => {
  if (!_0xc0638 || _0xc0638 == '') {
    _0x3f898a("  Exemple :* " + _0x338b80 + "boom FLASH-MD");
    return;
  }
  try {
    console.log(_0xc0638);
    var _0x140c55 = await mumaker.ephoto('https://en.ephoto360.com/boom-text-comic-style-text-effect-675.html', _0xc0638);
    _0x3f898a("  processing ...");
    await _0x154a8e.sendMessage(_0x117e9b, {
      'image': {
        'url': _0x140c55.image
      },
      'caption': " *Logo by FLASH-MD*"
    });
  } catch (_0x589367) {
    _0x3f898a("🥵🥵 " + _0x589367);
  }
});
france({
  'nomCom': "water",
  'categorie': "Logo",
  'reation': '💦'
}, async (_0x294867, _0x45a9fb, _0x15e9f6) => {
  if (!_0x4d7526 || _0x4d7526 == '') {
    _0x24a9b6(_0x5803c7 + "water france");
    return;
  }
  try {
    var _0x79230 = await mumaker.ephoto('https://en.ephoto360.com/create-water-effect-text-online-295.html', _0x4d7526);
    _0x24a9b6(" processing ...");
    await _0x45a9fb.sendMessage(_0x294867, {
      'image': {
        'url': _0x79230.image
      },
      'caption': "  *Logo by FLASH-MD*"
    });
  } catch (_0x5da0b3) {
    _0x24a9b6("🥵🥵 " + _0x5da0b3);
  }
});
france({
  'nomCom': "snow",
  'categorie': "Logo",
  'reaction': '❄️'
}, async (_0x4b2015, _0x3fa1bb, _0x1e333b) => {
  const {
    arg: _0x373fd2,
    ms: _0xa5142a,
    prefixe: _0x4fe976,
    repondre: _0x3240f8
  } = _0x1e333b;
  if (!_0x373fd2[0x0]) {
    _0x3240f8("Exemple of using commande:\n " + _0x4fe976 + "Snow My text");
    return;
  }
  const _0x15a111 = _0x373fd2.join(" ");
  mumaker.textpro("https://textpro.me/create-beautiful-3d-snow-text-effect-online-1101.html", _0x15a111).then(_0x3268a8 => {
    _0x3fa1bb.sendMessage(_0x4b2015, {
      'image': {
        'url': _0x3268a8.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0xa5142a
    });
  })["catch"](_0x4e86df => {
    console.error("Une erreur s'est produite :", _0x4e86df);
  });
});
france({
  'nomCom': "transformer",
  'categorie': "Logo",
  'reaction': '🤖'
}, async (_0x44f8c3, _0x405bca, _0x51482f) => {
  const {
    arg: _0x240b0e,
    ms: _0x3239e1,
    prefixe: _0xcaef94,
    repondre: _0x4fc8f7
  } = _0x51482f;
  if (!_0x240b0e[0x0]) {
    _0x4fc8f7("Exemple of using commande:\n " + _0xcaef94 + "Transformer My text");
    return;
  }
  const _0x1a18f0 = _0x240b0e.join(" ");
  mumaker.textpro("https://textpro.me/create-a-transformer-text-effect-online-1035.html", _0x1a18f0).then(_0x1f2535 => {
    _0x405bca.sendMessage(_0x44f8c3, {
      'image': {
        'url': _0x1f2535.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x3239e1
    });
  })["catch"](_0x556333 => {
    console.error("Une erreur s'est produite :", _0x556333);
  });
});
france({
  'nomCom': 'thunder',
  'categorie': "Logo",
  'reaction': '⚡'
}, async (_0x2369a7, _0x59c7af, _0x32a775) => {
  const {
    arg: _0x3d4f24,
    ms: _0x6e5183,
    prefixe: _0xc6573c,
    repondre: _0x5be6c4
  } = _0x32a775;
  if (!_0x3d4f24[0x0]) {
    _0x5be6c4("Exemple of using commande:\n " + _0xc6573c + "Thunder My text");
    return;
  }
  const _0x28e051 = _0x3d4f24.join(" ");
  mumaker.textpro("https://textpro.me/online-thunder-text-effect-generator-1031.html", _0x28e051).then(_0x512b24 => {
    _0x59c7af.sendMessage(_0x2369a7, {
      'image': {
        'url': _0x512b24.image
      },
      'caption': "Logo by *FLASH-MD*"
    }, {
      'quoted': _0x6e5183
    });
  })["catch"](_0x5a2d77 => {
    console.error("Une erreur s'est produite :", _0x5a2d77);
  });
});
france({
  'nomCom': "harrypotter",
  'categorie': 'Logo',
  'reaction': '🧙‍♂️'
}, async (_0x2834b8, _0x2b6c85, _0x73e6b9) => {
  const {
    arg: _0x223829,
    ms: _0x47aa3e,
    prefixe: _0x4445e4,
    repondre: _0x5cd1c8
  } = _0x73e6b9;
  if (!_0x223829[0x0]) {
    _0x5cd1c8("Exemple of using commande:\n " + _0x4445e4 + "HarryPotter My text");
    return;
  }
  const _0x5e0876 = _0x223829.join(" ");
  mumaker.textpro("https://textpro.me/create-harry-potter-text-effect-online-1025.html", _0x5e0876).then(_0x133eb0 => {
    _0x2b6c85.sendMessage(_0x2834b8, {
      'image': {
        'url': _0x133eb0.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x47aa3e
    });
  })["catch"](_0x5b9c8d => {
    console.error("Une erreur s'est produite :", _0x5b9c8d);
  });
});
france({
  'nomCom': 'cat',
  'categorie': "Logo",
  'reaction': '🪟'
}, async (_0x1c6a15, _0x5bd9f4, _0x38548d) => {
  const {
    arg: _0x37615e,
    ms: _0x1c57cd,
    prefixe: _0x52d82c
  } = _0x38548d;
  if (!_0x37615e[0x0]) {
    repondre("Exemple of using commande:\n " + _0x52d82c + "FoggyWindow My text");
    return;
  }
  const _0x460794 = _0x37615e.join(" ");
  mumaker.ephoto("https://en.ephoto360.com/handwritten-text-on-foggy-glass-online-680.html", _0x460794).then(_0x341d39 => {
    _0x5bd9f4.sendMessage(_0x1c6a15, {
      'image': {
        'url': _0x341d39.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x1c57cd
    });
  })['catch'](_0x31d1ea => {
    console.error("Une erreur s'est produite :", _0x31d1ea);
  });
});
france({
  'nomCom': "whitegold",
  'categorie': "Logo",
  'reaction': '💫'
}, async (_0x3314a7, _0x2f6f95, _0x2638d1) => {
  const {
    arg: _0x2c6c4c,
    ms: _0x39b4d5,
    prefixe: _0x3d9841,
    repondre: _0x32af56
  } = _0x2638d1;
  if (!_0x2c6c4c[0x0]) {
    _0x32af56("Exemple of using commande:\n " + _0x3d9841 + "WhiteGold My text");
    return;
  }
  const _0x2002d4 = _0x2c6c4c.join(" ");
  mumaker.textpro('https://textpro.me/elegant-white-gold-3d-text-effect-online-free-1070.html', _0x2002d4).then(_0xfe2c0d => {
    _0x2f6f95.sendMessage(_0x3314a7, {
      'image': {
        'url': _0xfe2c0d.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x39b4d5
    });
  })["catch"](_0x27bc86 => {
    console.error("Une erreur s'est produite :", _0x27bc86);
  });
});
france({
  'nomCom': "lightglow",
  'categorie': "Logo",
  'reaction': '🌟'
}, async (_0xae666b, _0x1d5b02, _0x389409) => {
  const {
    arg: _0x5995bf,
    ms: _0x1e6332,
    prefixe: _0x3cb4aa,
    repondre: _0x6aeed3
  } = _0x389409;
  if (!_0x5995bf[0x0]) {
    _0x6aeed3("Exemple of using commande:\n " + _0x3cb4aa + "LightGlow My text");
    return;
  }
  const _0x423d6b = _0x5995bf.join(" ");
  mumaker.textpro("https://textpro.me/create-light-glow-sliced-text-effect-online-1068.html", _0x423d6b).then(_0xa41469 => {
    _0x1d5b02.sendMessage(_0xae666b, {
      'image': {
        'url': _0xa41469.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x1e6332
    });
  })["catch"](_0x22fa33 => {
    console.error("Une erreur s'est produite :", _0x22fa33);
  });
});
france({
  'nomCom': "thor",
  'categorie': "Logo",
  'reaction': '🔨'
}, async (_0x3fc490, _0x5c27bb, _0x3364c3) => {
  const {
    arg: _0x2adb87,
    ms: _0x2f2375,
    prefixe: _0x299f8d,
    repondre: _0x2d36c7
  } = _0x3364c3;
  if (!_0x2adb87[0x0]) {
    _0x2d36c7("Exemple of using commande:\n " + _0x299f8d + "Thor My text");
    return;
  }
  const _0x25e750 = _0x2adb87.join(" ");
  mumaker.textpro('https://textpro.me/create-thor-logo-style-text-effect-online-1064.html', _0x25e750).then(_0x465666 => {
    _0x5c27bb.sendMessage(_0x3fc490, {
      'image': {
        'url': _0x465666.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x2f2375
    });
  })["catch"](_0x3969fd => {
    console.error("Une erreur s'est produite :", _0x3969fd);
  });
});
france({
  'nomCom': "neon",
  'categorie': "Logo",
  'reaction': '💡'
}, async (_0x4ab24e, _0x201a1c, _0x552601) => {
  const {
    arg: _0x53877d,
    ms: _0x256006,
    prefixe: _0x48ee90,
    repondre: _0x1475d0
  } = _0x552601;
  if (!_0x53877d[0x0]) {
    _0x1475d0("Exemple of using commande:\n " + _0x48ee90 + "Neon My text");
    return;
  }
  const _0x1843f9 = _0x53877d.join(" ");
  mumaker.textpro("https://textpro.me/neon-text-effect-online-879.html", _0x1843f9).then(_0x483373 => {
    _0x201a1c.sendMessage(_0x4ab24e, {
      'image': {
        'url': _0x483373.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x256006
    });
  })['catch'](_0x4df764 => {
    console.error("Une erreur s'est produite :", _0x4df764);
  });
});
france({
  'nomCom': "purple",
  'categorie': "Logo",
  'reaction': '🧳'
}, async (_0x492ad7, _0x3fee6e, _0x5760d2) => {
  try {
    if (!_0x24f439 || _0x24f439 == '') {
      _0x249bc6(_0x2691de + "purple Flash");
      return;
    }
    var _0x351a2d = await mumaker.ephoto("https://en.ephoto360.com/purple-text-effect-online-100.html", _0x24f439);
    _0x249bc6(" processing ...");
    await _0x3fee6e.sendMessage(_0x492ad7, {
      'image': {
        'url': _0x351a2d.image
      },
      'caption': "*Logo by FLASH-MD*"
    });
  } catch (_0x2f040f) {
    _0x249bc6(_0x2f040f);
  }
});
france({
  'nomCom': "gold",
  'categorie': "Logo",
  'reaction': "🧚🏿‍♀️"
}, async (_0x40df2a, _0x734a98, _0x33efa1) => {
  let {
    ms: _0x57a225,
    arg: _0x4aaadd,
    prefixe: _0x103d47,
    repondre: _0x22ece3
  } = _0x33efa1;
  try {
    if (!_0x4aaadd || _0x4aaadd == '') {
      _0x22ece3(_0x103d47 + "gold Flash-MD");
      return;
    }
    var _0xc8d9a0 = await mumaker.ephoto("https://en.ephoto360.com/modern-gold-4-213.html", _0x4aaadd);
    _0x22ece3("processing ...");
    await _0x734a98.sendMessage(_0x40df2a, {
      'image': {
        'url': _0xc8d9a0.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x57a225
    });
  } catch (_0x3202a4) {
    _0x22ece3(_0x3202a4);
  }
});
france({
  'nomCom': "arena",
  'categorie': 'Logo',
  'reaction': '🥵'
}, async (_0x8ce541, _0x24a66f, _0x29c2d9) => {
  let {
    ms: _0x4e115a,
    arg: _0x586124,
    prefixe: _0x1b846c,
    repondre: _0x13017b
  } = _0x29c2d9;
  try {
    if (!_0x586124 || _0x586124 == '') {
      _0x13017b(_0x1b846c + "arena Flash-MD");
      return;
    }
    var _0x2408ee = await mumaker.ephoto("https://en.ephoto360.com/create-cover-arena-of-valor-by-mastering-360.html", _0x586124.join(" "));
    _0x13017b("processing ...");
    await _0x24a66f.sendMessage(_0x8ce541, {
      'image': {
        'url': _0x2408ee.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x4e115a
    });
  } catch (_0x359f34) {
    _0x13017b(_0x359f34);
  }
});
france({
  'nomCom': "incandescent",
  'categorie': "Logo",
  'reaction': '😋'
}, async (_0x50ff31, _0x4e2cfb, _0xdb325b) => {
  let {
    ms: _0x5c0779,
    arg: _0x5f1a60,
    prefixe: _0x2b8316,
    repondre: _0x32974c
  } = _0xdb325b;
  try {
    if (!_0x5f1a60 || _0x5f1a60 == '') {
      _0x32974c(_0x2b8316 + "incandescent Flash-MD");
      return;
    }
    var _0x2234f3 = await mumaker.ephoto("https://en.ephoto360.com/text-effects-incandescent-bulbs-219.html", _0x5f1a60.join(" "));
    _0x32974c("processing ...");
    await _0x4e2cfb.sendMessage(_0x50ff31, {
      'image': {
        'url': _0x2234f3.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x5c0779
    });
  } catch (_0x136ab8) {
    _0x32974c(_0x136ab8);
  }
});
france({
  'nomCom': 'child',
  'categorie': "Logo",
  'reaction': '😇'
}, async (_0x245545, _0x3f0beb, _0x26173a) => {
  let {
    ms: _0xf6d56b,
    arg: _0x145757,
    prefixe: _0x672b9e,
    repondre: _0x2cb5e0
  } = _0x26173a;
  try {
    if (!_0x145757 || _0x145757 == '') {
      _0x2cb5e0(_0x672b9e + "child Flash-MD");
      return;
    }
    var _0xc3e079 = await mumaker.ephoto('https://en.ephoto360.com/write-text-on-wet-glass-online-589.html', _0x145757.join(" "));
    _0x2cb5e0("processing ...");
    await _0x3f0beb.sendMessage(_0x245545, {
      'image': {
        'url': _0xc3e079.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0xf6d56b
    });
  } catch (_0x59350b) {
    _0x2cb5e0(_0x59350b);
  }
});
france({
  'nomCom': 'typo',
  'categorie': "Logo",
  'reaction': '😇'
}, async (_0x3cc8d1, _0x1b49e1, _0x3b3c85) => {
  let {
    ms: _0x4b577f,
    arg: _0x3e1839,
    prefixe: _0x203b33,
    repondre: _0x1f7fb2
  } = _0x3b3c85;
  try {
    if (!_0x3e1839 || _0x3e1839 == '') {
      _0x1f7fb2(_0x203b33 + "typo Flash-MD");
      return;
    }
    var _0x3185ec = await mumaker.ephoto("https://en.ephoto360.com/typography-text-effect-on-pavement-online-774.html", _0x3e1839.join(" "));
    _0x1f7fb2("processing ...");
    await _0x1b49e1.sendMessage(_0x3cc8d1, {
      'image': {
        'url': _0x3185ec.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x4b577f
    });
  } catch (_0x34838d) {
    _0x1f7fb2(_0x34838d);
  }
});
france({
  'nomCom': 'light',
  'categorie': "Logo",
  'reaction': '🌠'
}, async (_0x3b2522, _0x245407, _0x15921c) => {
  let {
    ms: _0x36ae73,
    arg: _0x2cdfaa,
    prefixe: _0x49c690,
    repondre: _0x3542ba
  } = _0x15921c;
  try {
    if (!_0x2cdfaa || _0x2cdfaa == '') {
      _0x3542ba(_0x49c690 + "light Flash-MD");
      return;
    }
    var _0x373e9f = await mumaker.ephoto("https://en.ephoto360.com/light-text-effect-futuristic-technology-style-648.html", _0x2cdfaa.join(" "));
    _0x3542ba("processing ...");
    await _0x245407.sendMessage(_0x3b2522, {
      'image': {
        'url': _0x373e9f.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x36ae73
    });
  } catch (_0x4ada63) {
    _0x3542ba(_0x4ada63);
  }
});
france({
  'nomCom': "steel",
  'categorie': 'Logo',
  'reaction': '🌠'
}, async (_0x4dacf4, _0x1b87a2, _0x2d5750) => {
  let {
    ms: _0x2607f5,
    arg: _0x5c0015,
    prefixe: _0x131f7e,
    repondre: _0x5a2777
  } = _0x2d5750;
  try {
    if (!_0x5c0015 || _0x5c0015 == '') {
      _0x5a2777(_0x131f7e + "steel Flash-MD");
      return;
    }
    var _0x11adae = await mumaker.ephoto('https://en.ephoto360.com/dragon-steel-text-effect-online-347.html', _0x5c0015.join(" "));
    _0x5a2777("processing ...");
    await _0x1b87a2.sendMessage(_0x4dacf4, {
      'image': {
        'url': _0x11adae.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x2607f5
    });
  } catch (_0x5cb7d1) {
    _0x5a2777(_0x5cb7d1);
  }
});
france({
  'nomCom': 'sunlight',
  'categorie': "Logo",
  'reaction': '🌠'
}, async (_0x3436c3, _0x46f62f, _0x5132f1) => {
  let {
    ms: _0x5e026a,
    arg: _0x16f86c,
    prefixe: _0x4533db,
    repondre: _0x1f13a0
  } = _0x5132f1;
  try {
    if (!_0x16f86c || _0x16f86c == '') {
      _0x1f13a0(_0x4533db + "sunlight Flash-MD");
      return;
    }
    var _0x25c924 = await mumaker.ephoto('https://en.ephoto360.com/sunlight-shadow-text-204.html', _0x16f86c.join(" "));
    _0x1f13a0("processing ...");
    await _0x46f62f.sendMessage(_0x3436c3, {
      'image': {
        'url': _0x25c924.image
      },
      'caption': " *Logo by FLASH-MD*"
    }, {
      'quoted': _0x5e026a
    });
  } catch (_0x561db3) {
    _0x1f13a0(_0x561db3);
  }
});
france({
  'nomCom': "scifi",
  'categorie': "Logo",
  'reaction': '💥'
}, async (_0x29a36a, _0x2c5269, _0xfaf6e1) => {
  const {
    arg: _0x1ae38a,
    ms: _0x124cac,
    prefixe: _0x389552,
    repondre: _0x346c17
  } = _0xfaf6e1;
  if (!_0x1ae38a[0x0]) {
    _0x346c17("Example of using command:\n " + _0x389552 + "scifi My text");
    return;
  }
  const _0x35d3b8 = _0x1ae38a.join(" ");
  mumaker.textpro("https://textpro.me/create-3d-sci-fi-text-effect-online-1050.html", _0x35d3b8).then(_0x3b491f => {
    _0x2c5269.sendMessage(_0x29a36a, {
      'image': {
        'url': _0x3b491f.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x124cac
    });
  })["catch"](_0x312944 => {
    console.error("Une erreur s'est produite :", _0x312944);
  });
});
france({
  'nomCom': "magma",
  'categorie': "Logo",
  'reaction': '♨️'
}, async (_0x83ab4c, _0xefb08a, _0x56f3b4) => {
  const {
    arg: _0x26a0a2,
    ms: _0x862632,
    prefixe: _0x499ea1,
    repondre: _0x3ae0bd
  } = _0x56f3b4;
  if (!_0x26a0a2[0x0]) {
    _0x3ae0bd("Example of using command:\n " + _0x499ea1 + "magma My text");
    return;
  }
  const _0x554c81 = _0x26a0a2.join(" ");
  mumaker.textpro("https://textpro.me/create-a-magma-hot-text-effect-online-1030.html", _0x554c81).then(_0x545cb4 => {
    _0xefb08a.sendMessage(_0x83ab4c, {
      'image': {
        'url': _0x545cb4.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x862632
    });
  })["catch"](_0x59b90a => {
    console.error("Une erreur s'est produite :", _0x59b90a);
  });
});
france({
  'nomCom': "leaves",
  'categorie': 'Logo',
  'reaction': '☘️'
}, async (_0x3ba2a5, _0x47f7a8, _0xd0adb8) => {
  const {
    arg: _0x2732d9,
    ms: _0x9e4613,
    prefixe: _0x90da9c,
    repondre: _0x34c8b9
  } = _0xd0adb8;
  if (!_0x2732d9[0x0]) {
    _0x34c8b9("Example of using command:\n " + _0x90da9c + "leaves My text");
    return;
  }
  const _0x3aa090 = _0x2732d9.join(" ");
  mumaker.textpro('https://textpro.me/natural-leaves-text-effect-931.html', _0x3aa090).then(_0xe21133 => {
    _0x47f7a8.sendMessage(_0x3ba2a5, {
      'image': {
        'url': _0xe21133.image
      },
      'caption': "Logo BY *FLASH-MD*"
    }, {
      'quoted': _0x9e4613
    });
  })["catch"](_0x536d58 => {
    console.error("Une erreur s'est produite :", _0x536d58);
  });
});
